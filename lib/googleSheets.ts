...
export async function appendSheetData(range: string, values: any[][]) {
  ...
  try {
    // Determine the offset by looking at the first non-empty row
    const sheetName = extractSheetName(range);
    let offset = 0;
    if (sheetName) {
      ...
      const firstNonEmptyRow = allRows.find(row => row && row.some(cell => cell && cell.toString().trim() !== ''));
      
      if (firstNonEmptyRow) {
        while (offset < firstNonEmptyRow.length && (!firstNonEmptyRow[offset] || firstNonEmptyRow[offset].toString().trim() === '')) {
          offset++;
        }
      }
    }

    const paddedValues = values.map(row => Array(offset).fill('').concat(row));
